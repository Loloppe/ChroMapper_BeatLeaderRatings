...
namespace BeatLeaderRatings.AccAi
{
    internal class InferPublish
    {
        ...
        public double GetAIAcc(DifficultyV3 mapdata, double bpm, double njs, double timescale)
        {
            var (accs, noteTimes, freePoints) = PredictHitsForMap(mapdata, bpm, njs, timescale);
            double AIacc = GetMapAccForHits(accs, freePoints);
            if (timescale == 1) Console.WriteLine("Timescale: " + timescale);
            double adjustedAIacc = ScaleFarmability(AIacc, accs.Count, (noteTimes.Last() - noteTimes.First()) + 15);
            if (timescale == 1) Console.WriteLine("Before: " + AIacc);
            var mapLength = (noteTimes.Last() - noteTimes.First() + 4) / timescale + 2;
            adjustedAIacc = ScaleFarmability(AIacc, accs.Count, mapLength);
            AIacc = adjustedAIacc;
            if (timescale == 1) Console.WriteLine("After: " + AIacc);
            return AIacc;
        }
    }
}
